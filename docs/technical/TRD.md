...
SQLite stores queued, running, completed, failed, and quality-failed jobs. `BEGIN IMMEDIATE` provides atomic keyed creation and claim operations. `history.py` encodes strict versioned continuation cursors containing only a UTC timestamp and random UUID. `JobStore` implements indexed `(created_at DESC, report_job_id DESC)` keyset traversal with exact optional job-status filtering. The service calculates, invokes the selected interpreter, writes to a temporary artifact directory, and atomically renames it. FastAPI exposes calculation, enqueue, redacted history, individual status, deletion, and allow-listed artifact endpoints.

The report-job bounded context owns `report_job_id`, `job_status`, `report_request`, and `job_error_message` in Python. SQLite persists `report_job_id`, `job_status`, `request_json`, `request_fingerprint`, `idempotency_key_digest`, `created_at`, `updated_at`, `job_error_message`, and `artifact_dir`; these names are semantically specific and the table remains 3NF. Startup migrates legacy `id`, `status`, and `error` columns with SQLite `ALTER TABLE ... RENAME COLUMN` while holding the existing `BEGIN IMMEDIATE` schema transaction, then recreates affected indexes as `idx_report_jobs_job_status_created_at`, `idx_report_jobs_created_at_job_id`, and `idx_report_jobs_job_status_created_at_job_id`. The idempotency index remains `idx_report_jobs_idempotency_key_digest`. No queue row is duplicated and no hot-partition, reader/writer, UPSERT, retention, or artifact-path boundary changes.

The required `ReportJobRepository` remains backward compatible. Atomic keyed creation and history traversal are separate runtime-checkable capabilities, so an existing organization or MSA adapter can continue serving required operations and fails explicitly only when an unsupported optional endpoint is invoked. HTTP response compatibility is also preserved: the adapter serializes the established `id`, `status`, `error`, `artifacts`, and `items` keys through Pydantic aliases while the organization-owned domain and persistence contracts use semantic names.

A binary rollback across the persisted naming migration requires either restoring a pre-migration SQLite backup/snapshot or explicitly renaming the semantic columns back before an older binary is started; an older binary that still queries `id`, `status`, and `error` is not compatible with the advanced schema. Re-running the forward migration is idempotent because each rename is conditional on the legacy column being present and its semantic successor being absent.
...
Report-job naming regressions additionally require semantic Pydantic/domain fields, migration of legacy SQLite `id`/`status`/`error` columns without row loss, semantic index names, and continued serialization of the historical HTTP wire keys. A valid green result therefore proves both forward persistence migration and public compatibility rather than weakening either contract.
