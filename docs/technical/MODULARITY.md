...
`service.py` composes calculation, interpretation, quality validation, job state, and artifact publishing. `ReportService` accepts three required independent structural ports from `ports.py`:
...
Existing organization adapters remain compatible with normal creation, lookup, processing, deletion, and retention. Keyed creation or history traversal fails explicitly with HTTP 501 when the corresponding capability is absent; the service never substitutes process-local locking or process-local history for a distributed guarantee.

If no ports are supplied, the service creates the standalone `JobStore`, `NimReportInterpreter`, and `FilesystemArtifactPublisher` adapters. `JobStore` implements the base repository plus both optional capabilities. An integration may replace one adapter without replacing the others.
...
The top-level package exports the required structural ports and optional repository capabilities for integration type annotations:
...
A remote repository adapter must keep claim and transition operations atomic. If it implements `IdempotentReportJobRepository`, keyed creation must be atomic across all API instances. If it implements `ReportJobHistoryRepository`, every instance must return the same deterministic ordering for the same durable rows and must treat the cursor boundary as exclusive. A remote artifact publisher must finish all files in the staging location before returning, because the application service publishes the completed directory only after the port returns successfully.
...
`list_jobs` returns at most the requested limit in `(created_at DESC, id DESC)` order and an opaque continuation cursor only when a later row exists. The UUID is the deterministic tie-breaker when timestamps match. A supplied boundary is exclusive: rows must satisfy `created_at < boundary_created_at` or the equal-timestamp UUID comparison. An optional status filter is exact and remains part of the same stable traversal.

The cursor contains only a UTC timestamp and random job UUID. Integrations may use the package cursor codec or provide a transport-equivalent implementation, but they must reject malformed or unsupported cursors rather than guessing. New concurrent inserts may appear on a later first-page request but must not create duplicates inside an existing continuation sequence. Deletion and retention purge remove rows from future pages naturally.
...
Database objects created by this product use at least two words in `snake_case`; `camelCase` and `PascalCase` are accepted for external systems that require them. One-word identifiers and mixed underscore/capital styles are rejected by the product-gap audit. The standalone history indexes are `idx_report_jobs_created_id` and `idx_report_jobs_status_created_id`.

Public files and messages use UUID job identifiers rather than subject names. Artifacts remain under one validated job directory. Integrations must not put names, birth dates, user notes, raw idempotency keys, request fingerprints, or report copy in queue identifiers, history cursors, object keys, metrics labels, or log correlation identifiers.
...
A breaking calculation-policy change requires a new calculation version and new golden fixtures. A breaking report, required-port, or API schema change requires a major package version. Prompt wording changes require a prompt semantic-version update and evaluation evidence. Optional capabilities may be added in a minor version when the existing required port remains unchanged and unsupported use fails explicitly.
...
- history is exposed only when the repository implements `ReportJobHistoryRepository` with stable keyset ordering;
- collection items and cursors exclude stored requests, subject names, birth data, user notes, fingerprints, idempotency material, generated copy, traces, and artifact paths;
