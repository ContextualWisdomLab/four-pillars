...
2. **Interpretation purpose:** only the minimum deterministic evidence and user context required for the requested report may be sent to the explicitly selected interpretation backend. Provider choice is never inferred from data sensitivity or silently changed.
3. **History/status purpose:** collection/status APIs expose lifecycle and allow-listed artifact metadata only; they do not expose stored birth inputs, notes, report text, request fingerprints, idempotency material, model traces, or internal paths.
4. **Attribution/telemetry purpose:** organization attribution and ordinary telemetry must exclude subject labels, birth context, prompts, generated text, calculation fingerprints, artifact paths, and credentials.
...
|---|---|---|
| Birth date/time/timezone/location-derived context | Confidential personal data | deterministic calculation, requested interpretation, durable job processing |
| User context/relationship/work notes | Confidential personal data | requested interpretation only when supplied |
...
- If a history or artifact request is not authorized, it fails closed without revealing whether sensitive content exists beyond the minimum API contract.
