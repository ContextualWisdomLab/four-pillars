...
| Repository threat model | `docs/security/THREAT_MODEL.md` | Trust boundary, attacker-controlled input, security invariant, privileged workflow or material asset changes |
...
| Evidence/claim doctoring | `docs/doctoring/` including `documentation-governance.md` | Material external source, claim boundary, version-sensitive evidence or implementation mapping changes |
...
The repository already had substantial PRD, TRD, calculation/API/modularity, operations, standards, three ADRs, root architecture, UML, security guidance, Figma references, AGENTS/CLAUDE, and CHANGELOG coverage. The 2026-08-09 audit found and this baseline addresses the following documentation defects:
...
- there was no repository-scoped threat model enumerating assets, attacker-controlled inputs, invariants and runtime/control-plane threat boundaries;
- control-plane diagrams did not cover the minute-17 quality sentinel, minute-47 NVIDIA/OpenCode product-development workflow, or proposed minute-07 PR steward as separate authority zones;
...
- the TRD described Contextual Orchestrator native JSON response semantics inconsistently with production `ContextualOrchestratorClient(native_json_mode=False)` and required correction;
- standards references lacked architecture-description, requirements-engineering, UML, current privacy-management, information-security, SOC 2 Trust Services Criteria, and CSAP sources used by this baseline;
- the standards/assurance sources used by the baseline were not collected in one version-sensitive doctoring record.

## Shipped versus proposed

Architecture documents must label unmerged work explicitly. In particular, the minute-07 PR steward is **Proposed / under review** until its feature PR reaches protected `main`; diagrams may show it only with a Proposed marker. The minute-17 quality sentinel and minute-47 NVIDIA/OpenCode development loop are implemented protected-main control planes. Planned PostgreSQL/object-storage adapters, RFC 9457 responses, W3C Trace Context propagation, tenant/KMS/break-glass readiness controls, and deeper orchestration ablations remain Planned until separately reviewed implementation lands.
...
- the threat model remains repository-scoped and records assets, trust boundaries, untrusted inputs, invariants and residual gaps rather than turning into a current-PR finding list;
...
- material external sources are recorded in APA 7th style in `docs/standards/REFERENCES.md` and/or the relevant doctoring record;
- relevant tests/checks detect stale version, naming, file, ADR-status or high-risk contract assertions where practical.
