...
| GAP-13 | Hourly proposal publication has no configured, independent repository-scoped maintainer App boundary. | `planned` ([#34](https://github.com/ContextualWisdomLab/four-pillars/issues/34)) | Provision the documented least-privilege App or review a separately owned OIDC publication exchange; do not reuse review identities or add a PAT. | [presence-only configuration evidence](operations/WORKFLOW_REGISTRY_EVIDENCE.md), then a real protected-main zero-PR run creating at most one Draft PR whose normal exact-head gates start |
