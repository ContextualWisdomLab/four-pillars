...
| GAP-11 | A worker crash can leave a report job `running` indefinitely without a durable execution lease or race-safe reclaim. | `planned` ([#32](https://github.com/ContextualWisdomLab/four-pillars/issues/32)), dependency-gated | After #31 and #37 merge, implement versioned attempt ownership, heartbeat/expiry, atomic reclaim, stale-writer rejection, and attempt-scoped artifact publication without changing public job identity. | deterministic crash/reclaim and two-worker race tests, stale finish/fail rejection, restart persistence, idempotency preservation, redacted recovery state, 100% coverage |
| GAP-12 | Deleted one-shot and release-helper workflow identities remained active in the Actions registry; the owner loop disabled all 18 exact identities, but fleet recurrence detection is absent. | `planned` ([#33](https://github.com/ContextualWisdomLab/four-pillars/issues/33)) | Preserve the [protected-main-bound remediation receipt](operations/WORKFLOW_REGISTRY_EVIDENCE.md) and complete the linked organization/AppGuardrail read-only detector before closing the incident. | supported five workflows and dynamic CodeQL remain active; pagination/error/rename/reuse detector tests and post-integration fleet ledger remain required |
| GAP-13 | Hourly proposal publication has no configured independent maintainer App credentials at repository or inherited organization scope. | `planned` ([#34](https://github.com/ContextualWisdomLab/four-pillars/issues/34)) | Provision the documented least-privilege App or review a separately owned OIDC publication exchange; do not reuse review identities or add a PAT. | [presence-and-visibility-only configuration evidence](operations/WORKFLOW_REGISTRY_EVIDENCE.md), then a real protected-main zero-PR run creating at most one Draft PR whose normal exact-head gates start |
