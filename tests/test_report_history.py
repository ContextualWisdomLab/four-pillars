...
def test_history_cursor_rejects_naive_datetime_during_encoding() -> None:
    with pytest.raises(HistoryCursorError, match="Invalid report-history cursor"):
        encode_history_cursor(datetime(2026, 8, 4, 5, 6, 7), str(uuid.uuid4()))
...
def test_job_history_is_stable_newest_first_without_duplicate_rows(tmp_path: Path) -> None:
    ...
    timestamps = [base, base + timedelta(minutes=1), base + timedelta(minutes=1), base + timedelta(minutes=2), base + timedelta(minutes=3)]
    statuses = [
        JobStatus.QUEUED,
        JobStatus.COMPLETED,
        JobStatus.FAILED,
