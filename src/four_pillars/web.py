...
let reviewed=null, reportKey=null, pollTimer=null, historyCursor=null, historyLoading=false;
...
function restoreActiveJob(job){jobBox.hidden=false;downloads.replaceChildren();stateBox.textContent=`상태: ${statusLabels[job.status]||job.status}${job.error?` · ${job.error}`:''}`;if(['queued','running'].includes(job.status)){message('선택한 보고서 작업의 상태를 확인하고 있습니다.');poll(job.id)}else if(job.status==='completed'){renderCurrentArtifacts(job);message('완료된 보고서 파일을 선택할 수 있습니다.')}else{message('선택한 작업의 오류를 확인하세요.',true)}window.scrollTo({top:jobBox.getBoundingClientRect().top+window.scrollY-24,behavior:'smooth'})}
function renderHistoryItem(job){const item=document.createElement('article');item.className='history-item';const top=document.createElement('div');top.className='history-top';const identity=document.createElement('div');identity.className='history-identity';const chip=addText(identity,'span',statusLabels[job.status]||job.status,`history-chip status-${job.status}`);chip.setAttribute('aria-label',`작업 상태 ${statusLabels[job.status]||job.status}`);const id=addText(identity,'code',shortJobId(job.id),'history-id');id.title=job.id;top.appendChild(identity);addText(top,'time',historyTime(job.created_at),'history-time');item.appendChild(top);const bottom=document.createElement('div');bottom.className='history-bottom';let detail='대기 중인 작업입니다.';if(job.status==='running')detail='보고서를 생성하고 있습니다.';if(job.status==='completed')detail=job.artifacts.length?`사용 가능한 파일 ${job.artifacts.length}개`:'완료되었지만 공개된 파일이 없습니다.';if(['failed','quality_failed'].includes(job.status))detail=job.error||'작업을 완료하지 못했습니다.';addText(bottom,'p',detail,'history-detail');const actions=document.createElement('div');actions.className='history-actions';if(job.status==='completed'){job.artifacts.forEach(name=>artifactButton(actions,job.id,name))}else if(['queued','running'].includes(job.status)){const button=document.createElement('button');button.type='button';button.className='ghost';button.textContent='상태 보기';button.addEventListener('click',()=>restoreActiveJob(job));actions.appendChild(button)}bottom.appendChild(actions);item.appendChild(bottom);historyList.appendChild(item)}
async function loadHistory({reset=false}={}){if(historyLoading)return;if(reset){historyCursor=null;historyList.replaceChildren();historyMore.hidden=true}setHistoryLoading(true);historyMessage(reset?'최근 작업을 불러오고 있습니다.':'이전 작업을 더 불러오고 있습니다.');const params=new URLSearchParams({limit:String(historyLimit)});if(historyFilter.value)params.set('status',historyFilter.value);if(!reset&&historyCursor)params.set('cursor',historyCursor);try{const payload=await api(`/v1/reports?${params.toString()}`);if(reset)historyList.replaceChildren();payload.items.forEach(renderHistoryItem);historyCursor=payload.next_cursor;historyMore.hidden=!historyCursor;if(!historyList.children.length)addText(historyList,'p','표시할 최근 작업이 없습니다.','history-empty');historyMessage(payload.items.length?`${payload.items.length}개의 작업을 불러왔습니다.`:'더 표시할 작업이 없습니다.')}catch(error){if(reset)historyList.replaceChildren();historyCursor=null;historyMore.hidden=true;let text=error.message;if(error.status===401)text='API 키가 필요한 환경입니다. 키를 입력한 뒤 새로고침하세요.';if(error.status===501)text='현재 저장소는 최근 작업 조회를 지원하지 않습니다.';historyMessage(text,true)}finally{setHistoryLoading(false)}}
