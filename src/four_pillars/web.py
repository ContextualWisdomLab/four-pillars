...
<main>
  <section class="panel" aria-labelledby="input-title">
    ...
    <form id="report-form">
      ...
      <label>분석 월<input id="monthly-month" type="month" value="2026-08" required pattern="[0-9]{4}-(0[1-9]|1[0-2])" placeholder="YYYY-MM"><span class="hint">월 선택기를 지원하지 않는 브라우저에서는 YYYY-MM 형식으로 입력합니다.</span></label>
...
let reviewed=null, reportKey=null, pollTimer=null, pollRequest=0, historyCursor=null, historyLoading=false, historyRequest=0;
...
function clearCurrentJob(){clearTimeout(pollTimer);pollTimer=null;pollRequest+=1;jobBox.hidden=true;stateBox.textContent='';downloads.replaceChildren()}
async function poll(id){clearTimeout(pollTimer);pollTimer=null;const requestId=++pollRequest;try{const job=await api(`/v1/reports/${id}`);if(requestId!==pollRequest)return;jobBox.hidden=false;stateBox.textContent=`상태: ${statusLabels[job.status]||job.status}${job.error?` · ${boundedHistoryDetail(job.error)}`:''}`;if(job.status==='completed'){renderCurrentArtifacts(job);message('보고서 생성이 완료되었습니다.');loadHistory({reset:true});return}if(['failed','quality_failed'].includes(job.status)){downloads.replaceChildren();message('보고서를 완료하지 못했습니다. 상태와 오류를 확인하세요.',true);loadHistory({reset:true});return}pollTimer=setTimeout(()=>poll(id),2000)}catch(error){if(requestId!==pollRequest)return;message(boundedHistoryDetail(error.message),true)}}
...
apiKey.addEventListener('change',()=>{clearCurrentJob();loadHistory({reset:true})});
...
form.addEventListener('reset',()=>{setTimeout(()=>{reviewed=null;reportKey=null;generate.disabled=true;chartBox.hidden=true;raw.textContent='아직 계산하지 않았습니다.';message('입력을 초기화했습니다.');clearCurrentJob();loadHistory({reset:true})},0)});
function monthlyWindow(){const field=document.querySelector('#monthly-month'),match=/^(\d{4})-(0[1-9]|1[0-2])$/.exec(field.value.trim());return match?{field,year:Number(match[1]),month:Number(match[2])}:{field}}
form.addEventListener('submit',async event=>{event.preventDefault();if(!reviewed){await calculate();if(!reviewed)return}try{const window_=monthlyWindow();if(window_.year===undefined){window_.field.reportValidity();message('분석 월을 YYYY-MM 형식으로 입력해 주세요.',true);return}const year=window_.year,month=window_.month;if(!reportKey)reportKey=crypto.randomUUID();message('보고서 작업을 등록하고 있습니다…');const job=await api('/v1/reports',{method:'POST',headers:{'Idempotency-Key':`"${reportKey}"`},body:JSON.stringify({subject_name:document.querySelector('#subject').value,birth:birthPayload(),annual_year:Number(document.querySelector('#annual-year').value),monthly_year:year,monthly_month:month,user_context:document.querySelector('#context').value})});reportKey=null;jobBox.hidden=false;downloads.replaceChildren();stateBox.textContent=`상태: ${statusLabels[job.status]||job.status}`;loadHistory({reset:true});poll(job.id)}catch(error){message(error.message,true)}});
