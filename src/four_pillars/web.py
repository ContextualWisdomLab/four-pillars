...
function invalidBirthField(){return ['#birth-date','#timezone'].map(selector=>document.querySelector(selector)).find(field=>!field.checkValidity())}
async function calculate(){const invalid=invalidBirthField();if(invalid){invalid.reportValidity();message('계산에 필요한 항목을 채워 주세요.',true);return}try{message('만세력과 절기 경계를 계산하고 있습니다…');reviewed=await api('/v1/chart',{method:'POST',body:JSON.stringify(birthPayload())});renderChart(reviewed);generate.disabled=false;message('계산을 확인했습니다. 같은 입력으로 보고서를 생성할 수 있습니다.')}catch(error){reviewed=null;generate.disabled=true;message(error.message,true)}}
