...
let reviewed=null, reportKey=null, pollTimer=null;
...
async function poll(id){clearTimeout(pollTimer);try{const job=await api(`/v1/reports/${id}`);stateBox.textContent=`상태: ${job.status}${job.error?` · ${job.error}`:''}`;if(job.status==='completed'){downloads.replaceChildren();job.artifacts.forEach(name=>{const link=document.createElement('a');link.href=`/v1/reports/${id}/artifacts/${name}`;link.textContent=name;downloads.appendChild(link)});message('보고서 생성이 완료되었습니다.');return}if(['failed','quality_failed'].includes(job.status)){message('보고서를 완료하지 못했습니다. 상태와 오류를 확인하세요.',true);return}pollTimer=setTimeout(()=>poll(id),2000)}catch(error){message(error.message,true)}}
