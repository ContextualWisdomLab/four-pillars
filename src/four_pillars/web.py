...
function elapsedLabel(job){const started=Date.parse(job.created_at);if(!Number.isFinite(started))return '';const seconds=Math.max(0,Math.round((Date.now()-started)/1000));if(seconds<60)return ` · ${seconds}초 경과`;const minutes=Math.floor(seconds/60);if(minutes<60)return ` · ${minutes}분 경과`;return ` · ${Math.floor(minutes/60)}시간 ${minutes%60}분 경과`}
async function poll(id){clearTimeout(pollTimer);pollTimer=null;const requestId=++pollRequest;try{const job=await api(`/v1/reports/${id}`);if(requestId!==pollRequest)return;jobBox.hidden=false;stateBox.textContent=`상태: ${statusLabels[job.status]||job.status}${['queued','running'].includes(job.status)?elapsedLabel(job):''}${job.error?` · ${boundedHistoryDetail(job.error)}`:''}`;if(job.status==='completed'){renderCurrentArtifacts(job);message('보고서 생성이 완료되었습니다.');loadHistory({reset:true});return}if(['failed','quality_failed'].includes(job.status)){downloads.replaceChildren();message('보고서를 완료하지 못했습니다. 상태와 오류를 확인하세요.',true);loadHistory({reset:true});return}pollTimer=setTimeout(()=>poll(id),2000)}catch(error){if(requestId!==pollRequest)return;message(boundedHistoryDetail(error.message),true)}}
